...
parser = argparse.ArgumentParser("CIFAR-100 training")
...
parser.add_argument(
    ...
    help='sparsity of each conv layer, [0.]+[0.15]*2+[0.4]*27, [0.]+[0.4]*2+[0.5]*9+[0.6]*9+[0.7]*9')
        ...
        for k in range(num):
            for l in range(2):
                ...
                if orifilter_num != currentfilter_num:
                    ...
                    ci = np.load(prefix + str(cov_id) + subfix)
                    select_index = np.argsort(ci)[orifilter_num - currentfilter_num:]  # preserved filter id
                    # idx_range = np.arange(len(ci))
                    # select_index = np.random.permutation(idx_range[orifilter_num - currentfilter_num:]) # random preserved filter id
                    select_index.sort()
...
def main():
    ...
    ckpt = torch.load(args.pretrain_dir, map_location='cuda:0', weights_only=True)
